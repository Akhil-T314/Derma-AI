...
import { Activity, Mail, ArrowRight, ChevronLeft, CheckCircle2 } from "lucide-react";
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus("error");
      setMessage(err.response?.data?.error || "Failed to send reset link. Please check your email.");
    }
  };
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-gray-50 font-sans">
      <div className="absolute top-6 left-6 md:left-12">
        <Link to="/login" className="flex items-center text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        ...
        >
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="mt-2 text-gray-500 text-sm text-center">Enter your email to receive a password reset link</p>
          </div>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
            {status === "success" ? (
              <div className="text-center py-4">
                ...
                <Button asChild className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold h-11 rounded-lg shadow-md transition-all">
          ...
          <p className="mt-8 text-center text-xs text-gray-400 font-medium tracking-wide uppercase">
            DermAI Secure Authentication Service
          </p>
        </motion.div>
      </div>
    </div>
